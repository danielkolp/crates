...
export function getGemReasons(track = {}) {
  const reasons = []
  const releaseSignals = track.releaseMetadataSignals || {}
  const signals = Array.isArray(releaseSignals.signals) ? releaseSignals.signals : []
  const breakdown = track.scoreBreakdown || {}
  const flags = Array.isArray(track.flags) ? track.flags : []
  const views = safeNumber(track.views ?? track.viewCount, 0)
  const discoveredViaStyle = String(track.discoveredViaStyle || track.style || track.genre || '').trim()

  if (releaseSignals.isCleanAutoGeneratedRelease || flags.includes('clean-auto-generated-release')) {
    reasons.push('clean auto-generated release')
  } else if (signals.includes('topic-channel')) {
    reasons.push('clean Topic-channel release')
  } else if (signals.includes('provided-to-youtube-by') || signals.includes('released-on')) {
    reasons.push('official release metadata')
  }

  if (views > 0 && views <= 1000) {
    reasons.push('low exposure')
  } else if (safeNumber(breakdown.lowViewBoost, 0) >= 0.25) {
    reasons.push('underground visibility range')
  }

  if (safeNumber(breakdown.engagementBoost, 0) > 0.4 || safeNumber(breakdown.engagementScore, 0) >= 7.5) {
    reasons.push('strong engagement for its size')
  }

  if (safeNumber(breakdown.musicAuthenticityScore, 0) >= 7 || signals.includes('single-track-duration')) {
    reasons.push('clear music-track metadata')
  }

  if (track.styleConfidence === 'weak' && discoveredViaStyle) {
    reasons.push(`found through ${discoveredViaStyle} digging`)
  }

  if (reasons.length === 0 && String(track.gemReason || '').trim()) {
    reasons.push(...String(track.gemReason).split(';').map((reason) => reason.trim()).filter(Boolean))
  }
...
export function attachGemScores(tracks, profileOverrides = {}) {
  ...
  return tracks.map((track) => {
    ...
    const gemReasons = getGemReasons(scoredTrack)

    return {
      ...scoredTrack,
      gemReasons,
      gemReason: gemReasons.length > 0 ? gemReasons.join('; ') : scored.gemReason,
    }
  })
}
